 ...
 */
package com.movieverse.movieverse.store.web;

import com.movieverse.movieverse.store.ejb.ProductBean;
import com.movieverse.entities.Product;
import com.movieverse.movieverse.store.web.util.AbstractPaginationHelper;
...
public class ProductController implements Serializable {
    ...
    private DataModel searchResults = null;
    private DataModel items = null;
    @EJB
    private com.movieverse.movieverse.store.ejb.ProductBean ejbFacade;
...
//            current = ejbFacade.find(2);
    ...
    public  DataModel createSearchPageDataModel(){
         return new ListDataModel(getFacade().findByFulltextSearch(requestedItems));
    }
    
    public void doSearch(){
        searchResults = createSearchPageDataModel();
    }
    public String goSearch(){
        if(requestedItems != null){
              doSearch();
              logger.log(Level.INFO, "Search Worked!");
              logger.log(Level.INFO, searchResults.toString());
              logger.log(Level.INFO, ""+searchResults.getRowCount());
              //Product p = (Product) searchResults.getRowData();
